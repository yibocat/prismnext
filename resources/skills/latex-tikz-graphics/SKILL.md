---
name: latex-tikz-graphics
description: Use when drawing publication-quality TikZ/pgfplots figures — architecture and pipeline diagrams, commutative diagrams, function/schematic plots, and data line-plots rendered by LaTeX itself. Templates compile standalone into the project's figures folder.
license: MIT
---
...
- Any figure meant for print — paper, thesis, slides — pair with
  `scientific-visualization` when the data comes from experiment runs
...
3. **Edit incrementally, compile often** — `latex-compile` after each
   structural change; TikZ error messages point at the wrong line when you
   stack up edits.
4. **Include** — compile to PDF and `\includegraphics` the PDF, or move the
   picture into the manuscript with `input`; keep one source of truth.
...
- If a picture is mostly *data*, it belongs to `scientific-visualization`
