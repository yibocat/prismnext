# Baselines, Ablations, and the Reviewer Attack List

How comparisons stay fair, and how reviewers will try to break them.
...
2. **Tuning parity.** Baselines get a tuning budget comparable to yours.
   Report each baseline's tuning range and chosen config in the appendix.
   "Baseline with default hyperparameters" is visible to reviewers and fatal.
...
- The ablation matrix comes from `experiment-design-matrix`; each cell gets
...
## The reviewer attack list (preempt these)

- **Cherry-picked seeds** — you predefining the seed list is the defense;
  the experiment log is the receipt.
- **Untuned baseline** — tuning-parity appendix table is the defense.
- **Test-set peeking** — state the touch-once protocol in the experiment
  section.
...
- **"Difference within noise"** — the bold-best overlap check (below) and
  the statistical treatment from `statistical-rigor`.
...
rejects equality (`statistical-rigor` for the how). Overlapping CIs on the
top two cells → bold neither, or bold both with a footnote "difference not
significant". A bolded cell that loses this check is the single most
screenshotted reviewer complaint.
