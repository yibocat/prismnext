...
function StatusRow({
  ...
  const text: Record<Status, string> = {
    connecting: t("shell.status.connecting"),
    connected: t("shell.status.running"),
    disconnected: t("shell.status.stopped"),
    placeholder: "—",
  };
  return (
    <div className="flex items-center gap-2">
    ...
    <HoverCard openDelay={400} closeDelay={100}>
      <HoverCardTrigger asChild>
        ...
        >
          <CircleIcon
            ...
            aria-label={t("shell.status.system")}
          />
          {terminalAttention ? (
      ...
      <HoverCardContent side="bottom" align="start" className="w-52 p-3">
        <div className="space-y-2">
          <StatusRow label={t("shell.status.agent")} status={agentStatus} />
          <CompileStatusRow />
          <StatusRow
          ...
          />
          <StatusRow label={t("shell.status.mcp")} status="placeholder" />
          <StatusRow label={t("shell.status.lsp")} status="placeholder" />
        </div>
      </HoverCardContent>
    </HoverCard>
  );
