...
export const SYNTAX_THEMES: EditorSyntaxThemeDef[] = [
  ...
  },
  {
    ...
    getExtension: (mode: ThemeMode) => {
      ...
      return createPrismHighlightStyle(); // Nord is dark-only, fallback to prismnext for light
    },
  ...
  },
  {
    ...
    getExtension: (mode: ThemeMode) => {
      ...
      return createPrismHighlightStyle(); // fallback to prismnext for light mode
    },
    hasNativeVariant: (mode: ThemeMode) => mode === "dark",
  },
  {
 ...
 * For prismnext and oneDark (synchronous), returns immediately.
...
/**
 * Synchronous getter — only works for prismnext and oneDark (dark).
