...
function resolveDaemonWorkerPath(): string {
  const candidates = [
    ...
    join(getAppPath(), "out/main/tectonic-daemon-worker.mjs"),
    join(getAppPath(), "src/main/compile/tectonic-daemon-worker.mjs"),
  ];
  for (const p of candidates) {
    if (existsSync(p)) return p;
  }
  throw new Error(
    "tectonic-daemon-worker.mjs not found (checked out/main and src/main/compile)",
  );
}
