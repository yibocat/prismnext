 ...
 * Segment: stableSystem 1.4
 * Answers: local-first + chat vs disk + this project's functional folder map
 * Not here: compile how-to (manuscript-compile), experiment run how-to (experiments)
 * Settings: content is generated from Workspace folder config — not a free-text replace
 */
  ...
  const lines = dirs.map((d) => {
    const label =
      d.function === "custom" && "customLabel" in d
        ? (d as { customLabel?: string }).customLabel || FOLDER_FUNCTION_LABELS.custom
