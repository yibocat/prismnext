...
export const LITERATURE_LIBRARY_PROMPT = [
  ...
  "### Soft workflow",
  "",
  `- Library lookup: \`${TOOL_NAMES.literatureSearch}\` → \`${TOOL_NAMES.literatureRead}\` for metadata.`,
  `- **Topic / external papers (not in library):** use \`${TOOL_NAMES.literatureDiscover}\`, not \`${TOOL_NAMES.literatureSearch}\`.`,
