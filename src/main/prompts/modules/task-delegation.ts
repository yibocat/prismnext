...
export const TASK_DELEGATION_PROMPT = [
  ...
  "- Task `background` (boolean): set `true` when you can keep working (other tools / prose / more Tasks) before needing that result; omit or `false` when you must wait for the result immediately. Do not poll or sleep — OpenCode notifies you when a background Task finishes.",
  "- Wait for Task results before citing findings — do not invent results. For background Tasks, wait for the completion notification (not the early \"started\" stub).",
