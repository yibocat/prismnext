/**
 * interaction-list — List persisted Interaction specs in the project.
 */
...
function requestIdFrom(context: Record<string, unknown>): string {
  ...
  return `ix-list-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}
...
export default tool({
  ...
  args: {
    kindPrefix: tool.schema
      .string()
      .describe("Optional kind prefix filter, e.g. `plot` or `math.surface`.")
      .optional(),
  },
  async execute(args, context) {
    const kindPrefix = typeof args.kindPrefix === "string" ? args.kindPrefix.trim() : "";
