...
export const BUILTIN_TOOLS: BuiltinToolMeta[] = [
  ...
  },
  {
    ...
    workflowRules: [
      ...
      "Allowed kinds: figure.plotly, instrument, figure.static, plot.line, plot.series, plot.scatter, math.surface, math.field, scene.ir (legacy), scene.program (legacy builtin only).",
      "Scientific 2D/3D (surfaces, vector fields, heatmaps, step-through demos) → figure.plotly with spec.model.figure.",
      "Parametric surface/field needing LIVE recompute on binding change, or TRUE step-by-step iteration → instrument (not scene.ir/math.*, which are legacy static-per-write).",
