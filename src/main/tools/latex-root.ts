/**
 * latex-root — Resolve LaTeX main file, engine, and bib tool via prismnext bridge.
 */
...
async function bridgeCall(
  ...
  try { fs.unlinkSync(reqPath); } catch {}
  return toolOutput({ error: "LaTeX bridge timed out. Restart prismnext and try a new chat tab." });
}

export default tool({
  description: "Resolve the LaTeX main file, engine, bib tool, and build directory for this project.",
  args: {
    mainFile: tool.schema
      .string()
      .describe("Optional relative path to a .tex file; auto-detect when omitted")
      .optional(),
  },
