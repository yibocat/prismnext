...
function RenderMarkdown({ content }: { content: string }) {
  return (
    <ReactMarkdown remarkPlugins={[remarkCitationRefs]} urlTransform={(url) => url}>
      {content}
    </ReactMarkdown>
  );
}
...
describe("remarkCitationRefs", () => {
  ...
  it("turns [n] into a link node with citation: scheme", () => {
  ...
  it("transforms [n] inside table cells", () => {
    const md = "| # | Title |\n| --- | --- |\n| [1] | Foo |";
  ...
  it("transforms [n] inside list items", () => {
    const md = "- [1] first\n- [2] second";
    const { container } = render(<RenderMarkdown content={md} />);
  ...
  it("transforms multiple refs in one paragraph", () => {
