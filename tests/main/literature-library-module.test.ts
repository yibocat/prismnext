...
describe("LITERATURE_LIBRARY_PROMPT", () => {
  it("is library-citation workflow only — not external staging or tool catalog", () => {
    expect(LITERATURE_LIBRARY_PROMPT).toContain("library papers only");
    ...
    expect(LITERATURE_LIBRARY_PROMPT).toContain(TOOL_NAMES.literatureStage);
    ...
    expect(LITERATURE_LIBRARY_PROMPT).not.toContain("Prism Tools Guide");
  });
});
