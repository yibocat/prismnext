...
describe("buildManuscriptCompilePrompt", () => {
  it("keeps build workflow + boundaries; manuscript entry from workspace folder", () => {
    ...
    expect(latex).toContain("Workspace Folder Descriptions");
    expect(latex).toContain("No root-resolve tool");
    expect(latex).not.toContain("latex-root");
    expect(latex).not.toContain("typst-root");
  ...
  it("injects configured manuscript folder when present", () => {
    ...
    expect(latex).toContain("Workspace compile entry");
  });
  ...
  it("does not attach a main-file pin when the manuscript folder has none", () => {
    ...
    expect(latex).toContain("`paper/`");
    expect(latex).not.toContain("Workspace compile entry");
  });

  it("does not send standalone docs through the paper compile tool", () => {
    ...
    expect(paper?.promptGuidelines?.join(" ")).toContain("Workspace Folder Descriptions");

    const typstPaper = getNativeToolByName(TOOL_NAMES.typstCompile);
    expect(typstPaper?.promptGuidelines?.join(" ")).toContain("Workspace Folder Descriptions");
