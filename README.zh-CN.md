...
Agent 内置一套须遵守的科研规范——每条都配齐参考表、模板与可运行脚本：

| 层 | 内置规范 |
| --- | --- |
| **闭环编排** | 项目启动 · 相关工作流水线 · 精读笔记 · 假设设计 · 实验设计矩阵 · Methods 成文 · 图表流水线 · 结构化自审 · 审稿回复 · 投稿前总闸门 |
| **方法规范** | 统计严谨与功效分析 · PRISMA 2020 综述 · 色盲安全绘图 · SymPy 验证推导 · TikZ/pgfplots 模板 · 面板图视觉规范 |
| **学科协议** | 实证 ML 协议（种子、公平基线、聚合）· 管理与决策科学（DiD/IV/RDD、行为实验、稳健性） |
| **元** | 创作并校验你自己的 |

规范库随版本持续扩充——格式开放：可连接社区技能源，也可以写你自己的。
