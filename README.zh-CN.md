...
Agent 内置一套须遵守的科研规范——**29 个开箱技能**，每个都配齐参考表、模板与可运行脚本。每轮都必须成立的纪律放在常开 prompt 层；技能按需加载，且每个技能在所有同类被关闭时依然独立成立。

| 族 | 内置规范 |
| --- | --- |
| **构想** | idea-lab：评判缺席的发散头脑风暴 · 文献交叉授粉 · 专用 `ideas/` 文件夹，大胆火花不蒸发 |
| **设计与运行** | 假设设计 · 实验设计矩阵（算力成本先确认）· 实证 ML 协议（种子、公平基线、聚合）· 统计严谨与功效分析 · 管理与决策科学（DiD/IV/RDD、行为实验）· 运行收据 → Methods 正文 |
| **数学** | SymPy 验证推导 → LaTeX · 数值探针与收敛阶检验 · 微分几何与环/格验证——只验证新论断 |
| **图表** | matplotlib 模板 · Observable Plot 数据可视化（密度、hexbin、分面、地理）→ SVG · TikZ/pgfplots 矢量图 · 可复现图表流水线 · 面板可重开图 |
| **写作** | 动笔前的大纲闸门 · Introduction / 预备知识 / Methods / Results / Conclusion / Related Work 章节技能——模板是参照，不是模具 |
| **评审** | 精读笔记 · PRISMA 2020 · 反向角度批判性深审（配独立 peer-reviewer 专家）· 投稿前 preflight · 审稿回复 |
| **元** | 把刚刚走通的工作流蒸馏成你自己的技能 |

规范库随版本持续扩充——格式开放：可连接社区技能源，也可以写你自己的。Settings → Skills 会标识每个技能的来源（内置 / 注册源 / 自建），可单独开关或删除。
